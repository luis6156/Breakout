...
Game::Game(unsigned int width, unsigned int height)
    : State(GAME_MENU), Keys(), KeysProcessed(), Sequence(), PauseMusic(false), IsPaused(false), Width(width), Height(height), goodChance(20), badChance(16), Level(0), Lives(3)
{
...
void Game::Init()
{
    ...
    // audio
    BGSound = SoundEngine->play2D("audio/default_bg/breakout.mp3", true, false, true);
}
...
void Game::Update(float dt)
{
    ...
    {
        --this->Lives;
        // did the play4er lose all his lives? -> game over
        if (this->Lives == 0) 
        {
            this->ResetLevel();
    ...
    // check sound
    if (PauseMusic && !BGSound->getIsPaused())
    {
...
void Game::ProcessInput(float dt)
{
    ...
    {
        // Cheat code sequence (GOD)
        if (this->Keys[GLFW_KEY_G])
            this->Sequence[0] = true;
        if (this->Keys[GLFW_KEY_O] && this->Sequence[0])
            this->Sequence[1] = true;
        if (this->Keys[GLFW_KEY_D] && this->Sequence[0] && this->Sequence[1])
        {
            this->goodChance = 5;
            this->badChance = 12;
            Ball->Color = glm::vec3(1.0f, 0.0f, 0.0f);
            this->Sequence[0] = false;
            this->Sequence[1] = false;
        }
        // Select level
    ...
    if (this->State == GAME_ACTIVE)
    {
        ...
        // Go to Menu
        if (this->Keys[GLFW_KEY_ESCAPE]) // if the game is active, go to main menu
        {
            this->ResetLevel();
            this->ResetPlayer();
            this->State = GAME_MENU;
...
void Game::Render()
{
    ...
    if (this->State == GAME_MENU)
    {
        ...
        Text->RenderText("Press 9/0 to enable/disable VSYNC", 20.0f, this->Height - 20.0f, 0.6f);
        Text->RenderText("Press 8 to enable FPS counter", 500.0f, this->Height - 20.0f, 0.6f);
        Text->RenderText("Press M to mute/unmute music", 270.0f, this->Height / 2.0f + 100.0f, 0.6f);
        Text->RenderText("Press ESC to quit or enter main menu", 240.0f, this->Height / 2.0f + 80.0f, 0.6f);
    }
    if (this->State == GAME_WIN)
    {
...
void Game::ResetLevel()
{
    ...
    // Reset Power Ups
    this->PowerUps.erase(this->PowerUps.begin(), this->PowerUps.end());
...
{
    if (ShouldSpawn(this->goodChance)) // 1 in 20 chance
        this->PowerUps.push_back(PowerUp("speed", glm::vec3(0.5f, 0.5f, 1.0f), 0.0f, block.Position, ResourceManager::GetTexture("powerup_speed")));
    if (ShouldSpawn(this->goodChance))
        this->PowerUps.push_back(PowerUp("sticky", glm::vec3(1.0f, 0.5f, 1.0f), 20.0f, block.Position, ResourceManager::GetTexture("powerup_sticky")));
    if (ShouldSpawn(this->goodChance))
        this->PowerUps.push_back(PowerUp("pass-through", glm::vec3(0.5f, 1.0f, 0.5f), 10.0f, block.Position, ResourceManager::GetTexture("powerup_passthrough")));
    if (ShouldSpawn(this->goodChance))
        this->PowerUps.push_back(PowerUp("pad-size-increase", glm::vec3(1.0f, 0.6f, 0.4), 0.0f, block.Position, ResourceManager::GetTexture("powerup_increase")));
    if (ShouldSpawn(this->badChance)) // Negative powerups should spawn more often
        this->PowerUps.push_back(PowerUp("confuse", glm::vec3(1.0f, 0.3f, 0.3f), 15.0f, block.Position, ResourceManager::GetTexture("powerup_confuse")));
    if (ShouldSpawn(this->badChance))
